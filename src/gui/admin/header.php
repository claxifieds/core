...
<?php if (is_writable(osc_uploads_path())) { ?>
    <?php if ($logo_prefence) { ?>
        ...
        <img border="0" alt="<?php echo osc_esc_html(osc_page_title()); ?>"
             src="<?php echo bender_logo_url() . '?' . filemtime(osc_uploads_path() . osc_get_preference('logo', 'bender')); ?>"/>
        <form action="<?php echo osc_admin_render_theme_url('oc-content/themes/bender/admin/header.php'); ?>"
                ...
                <div class="form-horizontal">
                    <div class="form-actions">
                        <input id="button_remove" type="submit"
                               value="<?php echo osc_esc_html(__('Remove logo', 'bender')); ?>" class="btn btn-red">
                    </div>
                </div>
            </fieldset>
        </form>
    ...
    <?php if ($logo_prefence) { ?>
        <div class="flashmessage flashmessage-inline flashmessage-warning">
            <p><?php _e('<strong>Note:</strong> Uploading another logo will overwrite the current logo.', 'bender'); ?></p>
        </div>
    <?php } ?>
    <br/><br/>
    <form action="<?php echo osc_admin_render_theme_url('oc-content/themes/bender/admin/header.php'); ?>" method="post"
          enctype="multipart/form-data" class="nocsrf">
        ...
        <fieldset>
            <div class="form-horizontal">
                ...
                <div class="form-actions">
                    <input id="button_save" type="submit" value="<?php echo osc_esc_html(__('Upload', 'bender')); ?>"
